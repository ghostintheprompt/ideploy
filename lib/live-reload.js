  ...
  class FeedbackOverlay {
    ...
    createStyles() {
      ...
      style.textContent = `
        ...
        .ideploy-rec-btn.active { display: flex; }
        .ideploy-rec-btn.recording { background: #ff4444; animation: blink 1s infinite; }
    ...
    createUI() {
      ...
      this.recBtn.className = 'ideploy-rec-btn';
      this.recBtn.textContent = 'REC';
      document.body.appendChild(this.recBtn);
    ...
    bindEvents() {
      this.btn.addEventListener('click', () => {
        ...
        this.recBtn.classList.toggle('active', this.isActive);
    ...
    async toggleRecording() {
      if (this.isRecording) {
        ...
        try {
          ...
          console.error('[iDeploy] Screen record failed:', err);
        }
      }
    }
    ...
    async saveVideo() {
      ...
      formData.append('snapshot', JSON.stringify(this.getEnvironmentSnapshot()));

      console.log('[iDeploy] Uploading session recording...');
      try {
        await fetch(CONFIG.endpoint, { method: 'POST', body: formData });
      } catch (err) {
        console.error('[iDeploy] Failed to upload recording:', err);
      }
    }
